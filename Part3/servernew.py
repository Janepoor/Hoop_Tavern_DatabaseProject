...
def realadd():
	...
	if not is_safe(username) or not is_safe(password) or not is_safe(gender) or not is_safe(emailaddress) or not is_safe(agestr):
...
def validcheck():
	...
	if not is_safe(ninput) or not is_safe(pinput):
		msg='NO SQL INJECTION PLEASE'
		return render_template("error.html",msg=msg)
...
def useractualchange():
	...
	if not is_safe(newusername) or not is_safe(newpsd) or not is_safe(newgender) or not is_safe(newemailaddress) or not is_safe(newage):
		msg='NO SQL INJECTION PLEASE'
		return render_template("error.html",msg=msg)
	#fetch existing username data in the database as this would not be repeated
	username_existing=[]
	...
	#fetch existing emailaddress data in the database as this would not be repeated
	...
	if len(newusername)>0:
		if usernamejudge not in username_existing:
			cursor=g.conn.execute("update ltuser set username=%s where userid=%s", newusername, uid)
			cursor.close()
			print "UserID "+str(uid)+" is changing its username"
			uname=newusername
		else:
			...
			return render_template("useractualchange.html",**context)
	if len(newpsd)>0:
		cursor=g.conn.execute("update ltuser set password=%s where userid=%s", newpsd, uid)
		cursor.close()
		print "UserID "+str(uid)+" is changing its password"
	if len(newgender)>0:
		if(newgender=='f' or newgender=='F' or newgender=='M' or newgender=='m'):
			cursor=g.conn.execute("update ltuser set gender=%s where userid=%s",newgender.upper(),uid)
			cursor.close()
			print "UserID "+str(uid)+" is changing its gender"
		else:
			msg="illegal info update, please try again"
			context=dict(data=userdata1,uid=uid, uname=uname,message=msg)	
			return render_template("useractualchange.html",**context)		
	if len(newemailaddress)> 0:
		if emailaddressjudge not in emailaddress_existing and (newemailaddress.find('.com')>newemailaddress.find('@')+1 or newemailaddress.find('.edu')>newemailaddress.find('@')+1):
			cursor=g.conn.execute("update ltuser set emailaddress=%s where userid=%s", newemailaddress, uid)
			cursor.close()
			print "UserID "+str(uid)+" is changing its emailaddress"
		else:
			msg="illegal info update, please try again"
			context=dict(data=userdata1,uid=uid, uname=uname,message=msg)	
			return render_template("useractualchange.html",**context)		
	if len(newage)>0:
		if newage.isdigit() and int(newage)>0:
			cursor=g.conn.execute("update ltuser set age=%s where userid =%s", int(newage), uid)
			cursor.close()
			print "UserID "+str(uid)+" is changing its age"
		else:
			msg="illegal info update, please try again"
			context=dict(data=userdata1,uid=uid, uname=uname,message=msg)	
			return render_template("useractualchange.html",**context)
	elif not newusername and not newpsd and not newemailaddress and not newgender and not newage:
		message="Your Profile stays the same"
		context=dict(uid=uid, uname=uname, message=message)
		return render_template("dashboard.html",**context)		
...
def commentcheck():
	...
	print newrate
	if not is_safe(newcontent) or not is_safe(newrate):
		msg='NO SQL INJECTION PLEASE'
		return render_template("error.html", msg=msg)
...
def user_view_recommendations():
	...
	for elem in cursor:
		 agerank=int(elem[0])
		 genderrank=str(elem[1]).strip()
	cursor.close()
	print agerank,genderrank
	if not searchrank or not raterank and genderrank.upper()=='M':
		context=dict(uid=uid, tname='Heat')
		return render_template("user_view_recommendations.html",**context)
	elif not searchrank or not raterank and genderrank.upper()=='F':
		context=dict(uid=uid, tname='Ducks')
		return render_template("user_view_recommendations.html",**context)
	...
	if highest_search_times>=30: # massive search times threshold=30
		...
		if agerank<16 and genderrank.upper()=='M': team_recommend=='Heat'
		elif agerank<16 and genderrank.upper()=='F': team_recommend=='Ducks'
	else: team_recommend=team_highest
